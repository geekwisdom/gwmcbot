...
import org.geekwisdom.gwmcbot.network.protocol.login.*;
import org.geekwisdom.gwmcbot.network.protocol.play.*;
...
public class NetworkHandler {
    ...
    public NetworkHandler() {
        try {
            this.out = new DataOutputStream(GWmcbot.getInstance().getSocket().getOutputStream());
            this.in = new DataInputStream(GWmcbot.getInstance().getSocket().getInputStream());

            this.state = State.HANDSHAKE;
            initPacketRegistries();
        } catch (IOException e) {
            GWmcbot.getLog().severe("Could not start bot: " + e.getMessage());
        }
    }
    ...
    private void initPacketRegistries() {
        ...
        if(!ProtocolConstants.SUPPORTED_VERSION_IDS.contains(GWmcbot.getInstance().getServerProtocol())) {
            GWmcbot.getLog().severe("This server is not running a supported protocol version: ProtocolVersion " + GWmcbot.getInstance().getServerProtocol());
            GWmcbot.getLog().severe("It is possibe that it wont work correctly");

            getPlayRegistryIn().put(GWmcbot.getInstance().getServerProtocol(), new PacketRegistry());
            getPlayRegistryOut().put(GWmcbot.getInstance().getServerProtocol(), new PacketRegistry());
            getPlayRegistryIn().get(GWmcbot.getInstance().getServerProtocol()).copyOf(getPlayRegistryIn().get(ProtocolConstants.MINECRAFT_1_15_1));
            getPlayRegistryOut().get(GWmcbot.getInstance().getServerProtocol()).copyOf(getPlayRegistryOut().get(ProtocolConstants.MINECRAFT_1_15_1));
        }
    }
    ...
    public void sendPacket(Packet packet) {
        ...
        switch (getState()) {
            ...
            case LOGIN:
                ...
                break;
            case PLAY:
                Packet.writeVarInt(getPlayRegistryOut().get(GWmcbot.getInstance().getServerProtocol()).getId(packet.getClass()), buf);
                break;
            default:
                return;
        }
        ...
        //Add packet payload
        try {
            packet.write(buf, GWmcbot.getInstance().getServerProtocol());
        } catch (IOException e) {
            GWmcbot.getLog().warning("Could not instantiate " + packet.getClass().getSimpleName());
        }
        ...
        if (getThreshold() >= 0) {
            ...
            try {
                ...
                GWmcbot.getLog().severe("Error while trying to send: " + packet.getClass().getSimpleName());
            }
        } else {
        ...
        if (GWmcbot.getInstance().getConfig().isLogPackets())
            GWmcbot.getLog().info("[" + getState().name().toUpperCase() + "]  C  >>> |S|: " + packet.getClass().getSimpleName());
    }
    ...
    private void readPacket(int len, int packetId, ByteArrayDataInputWrapper buf) throws IOException {
        ...
        switch (state) {
            ...
            case LOGIN:
                ...
                break;
            case PLAY:
                clazz = getPlayRegistryIn().get(GWmcbot.getInstance().getServerProtocol()).getPacket(packetId);
                break;
            default:
                return;
        }
        ...
        if (clazz == null) {
            if (GWmcbot.getInstance().getConfig().isLogPackets())
                GWmcbot.getLog().info("[" + getState().name().toUpperCase() + "] |C| <<<  S : 0x" + Integer.toHexString(packetId));
            return;
        } else if (GWmcbot.getInstance().getConfig().isLogPackets())
            GWmcbot.getLog().info("[" + getState().name().toUpperCase() + "] |C| <<<  S : " + clazz.getSimpleName());

        try {
            Packet packet = clazz.newInstance();
            packet.read(buf, this, len, GWmcbot.getInstance().getServerProtocol());
        } catch (InstantiationException | IllegalAccessException e) {
            GWmcbot.getLog().warning("Could not create new instance of " + clazz.getSimpleName());
            e.printStackTrace();
        }
    }
    ...
    public void activateEncryption() {
        try {
            ...
            BufferedOutputStream var1 = new BufferedOutputStream(CryptManager.encryptOuputStream(getSecretKey(), GWmcbot.getInstance().getSocket().getOutputStream()), 5120);
    ...
    public void decryptInputStream() {
        ...
        try {
            InputStream var1;
            var1 = GWmcbot.getInstance().getSocket().getInputStream();
